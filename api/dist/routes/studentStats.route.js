...
var app = express();
var studentStatsRoutes = express.Router();
var StudentStats = require('../models/studentStats');
// Create new stat entry in database
studentStatsRoutes.route('/create').post(function (req, res) {
    var stat = new StudentStats(req.body);
...
studentStatsRoutes.route('/getStatsByClassroom/:id').get(function (req, res) {
    StudentStats.find({ classroomId: req.params.id }, function (err, stats) {
        if (err) {
            console.log(err, new Date());
            res.json(err);
        }
...
studentStatsRoutes.route('/updateGrammarErrors/:id/:updatedTimeStamp').post(function (req, res) {
    StudentStats.findOne({ "studentId": req.params.id }, function (err, stat) {
        ...
        if (!stat) {
            ...
            var newErrors = Object.entries(req.body);
            console.log(newErrors);
            //console.log(stat.grammarErrors);
            // loop through db errors
            for (var _i = 0, _a = stat.grammarErrors; _i < _a.length; _i++) {
                var entry = _a[_i];
                var originalAmount = entry[1];
                var dateArray = [];
                ...
                for (var i = 0; i < newErrors.length; i++) {
                    if (entry[0] === newErrors[i][1][0]) {
                        ...
                        var index = newErrors.indexOf(newErrors[i]);
                        if (index > -1) {
...
studentStatsRoutes.route('/delete/:id').get(function (req, res) {
    StudentStats.findOneAndRemove({ "studentId": req.params.id }, function (err, stat) {
        if (err) {
            ...
            res.json("Successfully removed stat for studentId");
    });
});
...
studentStatsRoutes.route('/deleteForClassroom/:id').get(function (req, res) {
    StudentStats.deleteMany({ "classroomId": req.params.id }, function (err, stat) {
        if (err) {
            ...
            res.json("Successfully removed stats for classroom id");
    });
});
