...
async function callAbairServer() {
  ...
  await Promise.all(
    Array(n)
      .fill(undefined)
      .map(async () => {
        return axios.get(request(fromObject)).then(
          (r) => {
            success++;
            console.log(r.data);
          },
 ...
 * http://localhost:4000/proxy => https://api.abair.ie/v3/synthesis?
 */
async function callLocalServer() {
  ...
  await Promise.all(
    Array(n)
      .fill(undefined)
      .map(async () => {
        return axios.post(localUrl, { url: body }, config).then(
          (r) => {
            success++;
            console.log(r.data);
          },
...
function request(params) {
  var queryString = Object.keys(params)
    ...
    .join("&");
  return "https://www./synthesise?" + queryString;
}
