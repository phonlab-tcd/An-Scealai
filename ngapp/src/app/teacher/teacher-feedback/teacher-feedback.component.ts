...
export class TeacherFeedbackComponent implements OnInit {
  ...
  createComment() {
    ...
    if (commentInput != null && commentInput !== "") {
      ...
      this.addComment(commentObj.comment);
    }
  }
  ...
  addComment(comment: string) {
    ...
    if (commentContainer) {
      ...
      buttonElement.addEventListener("click", (event) =>
        this.highlightCommentReferenceInQuill(event.target as HTMLElement)
      );
   ...
   */
  highlightCommentReferenceInQuill(commentElement: HTMLElement) {
  ...
  showInlineCommentButton(event) {
    ...
    if (range && range.length > 0 && event.source == "user") {  // don't want to create button when showCommentInQuill() is fired
      ...
      iconElement.classList.add("fa", "fa-envelope");
  ...
  /**
   * Remove any existing comment buttons from the DOM
   */
  ...
  openUpdateStoryTextDialog() {
    this.dialogRef = this.dialog.open(BasicDialogComponent, {
      data: {
        ...
        message: this.ts.l.student_has_made_changes_since_feedback.replace(
          "#",
          this.story.author
        ),
