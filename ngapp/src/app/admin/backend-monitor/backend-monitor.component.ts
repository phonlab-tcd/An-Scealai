...
import Chart from "chart.2.2.2.js";
import { io } from "socket.io-client.4.4.1.js";


function dynamic_module(s) {
	function maybe_default(m) {
		return m.default || m;
	}
	return import(s).then(maybe_default);
}
...
export class BackendMonitorComponent implements OnInit {
  ...
  expressMonitorObject: SafeResourceUrl;
	...
	heartbeat() {
		...
		setTimeout(heartbeat_recurse.bind(this),1000);
	}
