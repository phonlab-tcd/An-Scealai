...
export class StatsService {
  ...
  updateGrammarErrors(id: string, data: Map<string, HighlightTag[]>, updatedTimeStamp: Date): Observable<any> {
  ...
  updateStudentUsername(id: string, username: string): Observable<any> {
    return this.http.post(
      this.baseUrlStudents + 'updateStudentUsername/' + id,
      {username});
  }
  ...
  // Return an object containing grammar rule name,
  ...
  listErrors(): { error: string, description: string, type: string, checked: boolean }[]  {
    return [
      ...
      {
        error: 'NEAMHCHOIT',
        description: 'Valid word but extremely rare in actual usage. Is this the word you want?',
        type: 'GENERAL',
        checked: false,
      },
      ...
      {
        error: 'WEAKSEIMHIU',
        description: 'Often this preposition causes lenition, but this case is unclear',
        type: 'LENITION',
        checked: false},
      ...
      {
        error: 'SYNTHETIC',
        description: 'The synthetic (combined) form, with a given ending, is often used here',
        type: 'ECLIPSIS',
        checked: false},
      ...
      {
        error: 'PYMPESTREYLYANSTH',
        description: 'Fifth (mixed) mutation after \'th\' missing',
        type: 'SPECIFICLANGUAGE',
        checked: false},
      {
        error: 'VOWELHARMONY',
        description: 'This word violates the rules of Igbo vowel harmony',
        type: 'SPECIFICLANGUAGE',
        checked: false},
    ];
  }
