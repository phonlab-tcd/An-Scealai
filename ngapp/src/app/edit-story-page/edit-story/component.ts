...
export class EditStoryComponent implements OnInit{
  ...
  @ViewChild('mySynthesisPlayer') synthesisPlayer: SynthPlayerComponent;
  ...
  quillEditor: Quill;
  quillEditor$ = new Subject<Quill>();
  quillEditorReady = new Promise<Quill>(r=>{this.quillEditor$.subscribe({next: e => r(e)})});
  ...
  ngOnInit() {
    ...
    this.getStories().then(stories => {
      ...
      this.getStoryId().then(async params => {
        this.id = params.id;
        ...
        const story = this.stories.filter(s=>s._id === this.id)[0];
        ...
        (await this.quillEditorReady).root.innerHTML = this.story.htmlText;
